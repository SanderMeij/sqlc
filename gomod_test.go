...
// https://github.com/SanderMeij/sqlc/issues/4397.
//
// When go.mod contains a replace directive, the Go toolchain refuses to run
// `go install github.com/SanderMeij/sqlc/cmd/sqlc@latest` (and the equivalent
// `go run ...@latest`):
//
//	go: github.com/SanderMeij/sqlc/cmd/sqlc@latest (in github.com/SanderMeij/sqlc@v...):
...
func TestGoModHasNoReplaceDirectives(t *testing.T) {
	...
	if len(offenders) > 0 {
		t.Fatalf("go.mod must not contain replace directives; "+
			"they break `go install github.com/SanderMeij/sqlc/cmd/sqlc@latest`.\n"+
			"See https://github.com/SanderMeij/sqlc/issues/4397\n%s",
